...
get_sources(){
    ...
    wget ${PKG_RAW_URL}/common/etcd/etcd-server.install
    wget ${PKG_RAW_URL}/common/etcd/rules
    wget ${PKG_RAW_URL}/common/etcd/etcd-client.install
    wget ${PKG_RAW_URL}/common/etcd/etcd.service
    wget ${PKG_RAW_URL}/common/etcd/etcd.conf.yaml
    mv etcd.conf.yaml ../
    ...
    mkdir rpm
    cd rpm
    wget ${PKG_RAW_URL}/common/etcd/etcd.spec
    wget ${PKG_RAW_URL}/common/etcd/etcd.service
    wget ${PKG_RAW_URL}/common/etcd/etcd.conf.yaml
    cd ${WORKDIR}
    #
...
build_srpm(){
    ...
    rpmbuild -bs \
        ...
        --define "dist .generic" \
        --define "version ${ETCD_VERSION}" \
        --define "release ${ETCD_RELEASE}" \
        rpmbuild/SPECS/etcd.spec
...
if [ $INSTALL = 0 ]; then
    ...
    source install-deps.sh "etcd" "${PG_MAJOR}"
fi
get_sources
build_srpm
build_source_deb
