...
get_sources(){
    ...
    mkdir debian
    cd debian/
    wget ${PKG_RAW_URL}/common/pgbadger/control
    wget ${PKG_RAW_URL}/common/pgbadger/rules
    chmod +x rules
    wget ${PKG_RAW_URL}/common/pgbadger/copyright
    echo 9 > compat
    ...
    mkdir rpm
    cd rpm
    wget ${PKG_RAW_URL}/common/pgbadger/percona-pgbadger.spec
    cd ${WORKDIR}
    #
...
if [ $INSTALL = 0 ]; then
    ...
    source install-deps.sh "pgbadger" "${PG_MAJOR}"
fi
get_sources
build_srpm
build_source_deb
