...
get_sources(){
    ...
    rm -f control rules
    wget ${PKG_RAW_URL}/common/pgbouncer/control
    wget ${PKG_RAW_URL}/common/pgbouncer/rules
    wget ${PKG_RAW_URL}/common/pgbouncer/preinst
    echo 9 > compat
    cd ../
    rm -rf deb_packaging
    mkdir rpm
    cd rpm
    wget ${PKG_RAW_URL}/common/pgbouncer/percona-pgbouncer.spec
    wget ${PKG_RAW_URL}/common/pgbouncer/pgbouncer-ini.patch
    wget ${PKG_RAW_URL}/common/pgbouncer/pgbouncer.logrotate
    wget ${PKG_RAW_URL}/common/pgbouncer/pgbouncer.service
    wget ${PKG_RAW_URL}/common/pgbouncer/pgbouncer.service.rhel7
    wget ${PKG_RAW_URL}/common/pgbouncer/pgbouncer.sysconfig
    wget ${PKG_RAW_URL}/common/pgbouncer/pgbouncer.init
    cd ${WORKDIR}
    #
...
if [ $INSTALL = 0 ]; then
    ...
    source install-deps.sh "pgbouncer" "${PG_MAJOR}"
fi
get_sources
build_srpm
build_source_deb
