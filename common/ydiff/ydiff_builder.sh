...
get_sources(){
    ...
    cd debian
    wget ${PKG_RAW_URL}/common/ydiff/debian/rules
    wget ${PKG_RAW_URL}/common/ydiff/debian/changelog
    wget ${PKG_RAW_URL}/common/ydiff/debian/compat
    wget ${PKG_RAW_URL}/common/ydiff/debian/control
    wget ${PKG_RAW_URL}/common/ydiff/debian/copyright
    wget ${PKG_RAW_URL}/common/ydiff/debian/docs
    wget ${PKG_RAW_URL}/common/ydiff/debian/watch
    cd source
    wget ${PKG_RAW_URL}/common/ydiff/debian/source/format
    cd ../../

    export DEBIAN=$(lsb_release -sc)
    mkdir rpm
    cd rpm
    wget ${PKG_RAW_URL}/common/ydiff/rpm/ydiff.spec
    cd ../
    cd ${WORKDIR}
...
if [ $INSTALL = 0 ]; then
    ...
    source install-deps.sh "ydiff" "${PG_MAJOR}"
fi
get_sources
build_srpm
build_source_deb
