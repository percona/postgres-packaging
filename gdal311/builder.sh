...
get_srpm(){
    ...
    rpmdev-setuptree
    latest_rpm="gdal311-3.11.0-2PGDG.rhel9.src.rpm"
    
    cd ${WORKDIR}/rpmbuild/SRPMS/
    wget https://dnf-srpms.postgresql.org/srpms/common/redhat/rhel-9-x86_64/gdal311-3.11.0-2PGDG.rhel9.src.rpm
    latest_rpm=$(find . | grep gdal | grep src.rpm | cut -f2 -d'/')
    rpm2cpio $latest_rpm | cpio -id
    rm -f $latest_rpm
    mv gdal311.spec ../SPECS/
    mv * ../SOURCES/
    cd ../SPECS
    sed -i 's/PGDG//' gdal311.spec
    cd ../../
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" rpmbuild/SPECS/gdal311.spec
...
build_rpm(){
    ...
    SRC_RPM=$(basename $(find $WORKDIR/srpm -name 'gdal*.src.rpm' | sort | tail -n1))
    if [ -z $SRC_RPM ]
    then
        SRC_RPM=$(basename $(find $CURDIR/srpm -name 'gdal*.src.rpm' | sort | tail -n1))
        if [ -z $SRC_RPM ]
        then
