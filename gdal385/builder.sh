...
get_srpm(){
    ...
    cd $WORKDIR
    PRODUCT=gdal
    echo "PRODUCT=${PRODUCT}" > gdal385.properties
    GIT_USER=$(echo ${REPO} | awk -F'/' '{print $4}')

    PRODUCT_FULL=${PRODUCT}-${VERSION}
    echo "PRODUCT_FULL=${PRODUCT_FULL}" >> gdal385.properties
    echo "VERSION=${VERSION}" >> gdal385.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> gdal385.properties
    echo "BUILD_ID=${BUILD_ID}" >> gdal385.properties

    source gdal385.properties
    DATE_TIMESTAMP=$(date +%F_%H-%M-%S)
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${DATE_TIMESTAMP}/${BUILD_ID}" >> gdal385.properties

    rm -fr rpmbuild
    #
    mkdir -vp rpmbuild/{SOURCES,SPECS,BUILD,SRPMS,RPMS}
    rpmdev-setuptree
    latest_rpm="gdal38-3.8.5-7PGDG.rhel8.src.rpm"
    
    cd ${WORKDIR}/rpmbuild/SRPMS/
    wget https://dnf-srpms.postgresql.org/srpms/common/redhat/rhel-8-x86_64/gdal38-3.8.5-7PGDG.rhel8.src.rpm
...
CURDIR=$(pwd)
VERSION_FILE=$CURDIR/gdal385.properties
args=
WORKDIR=
SRPM=0
RPM=0
