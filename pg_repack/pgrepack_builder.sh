...
get_sources(){
    ...
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/Makefile.patch
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/rules
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/control
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/control.in
    ...
    mv ../control.in ./
    cd ../
    echo 14 > debian/pgversions
    echo 9 > debian/compat
    rm -rf deb_packaging
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/pg_repack.spec
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/pg_repack-pg14-makefile-pgxs.patch
    cd ../
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/14.0/pg_repack/make.patch
    patch -p0 < make.patch
    rm -f make.patch
    cd ${WORKDIR}
    #
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      apt-get update || true
      INSTALL_LIST="dpkg-dev build-essential percona-postgresql-14 debconf debhelper devscripts dh-exec git wget libkrb5-dev libssl-dev percona-postgresql-common percona-postgresql-server-dev-all"
