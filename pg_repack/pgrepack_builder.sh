...
get_sources(){
    ...
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/Makefile.patch
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/rules
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/control
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/control.in
    ...
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/pg_repack.spec
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/pg_repack-pg13-makefile-pgxs.patch
    cd ../
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/pg_repack/make.patch
    patch -p0 < make.patch
    rm -f make.patch
    cd ${WORKDIR}
    #
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      apt-get update || true
      INSTALL_LIST="dpkg-dev build-essential percona-postgresql-13 debconf debhelper devscripts dh-exec git wget libkrb5-dev libssl-dev percona-postgresql-common percona-postgresql-server-dev-all"
