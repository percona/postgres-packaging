...
get_sources(){
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${BRANCH}/${REVISION}/${DATE_TIMESTAMP}/${BUILD_ID}" >> pg_tde.properties
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
        ...
        if [ x"$RHEL" = x6 -o x"$RHEL" = x7 ]; then
            ...
            INSTALL_LIST="clang-devel clang llvm-devel json-c-devel libcurl-devel openssl-devel gettext git percona-postgresql${PG_MAJOR_VERSION}-devel percona-postgresql${PG_MAJOR_VERSION}-server rpmdevtools vim wget"
            yum -y install ${INSTALL_LIST}
