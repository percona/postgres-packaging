...
add_percona_yum_repo(){
    ...
    percona-release enable ppg-11.12 release
    return
}
...
get_sources(){
    ...
    PRODUCT=percona-pgaudit11_set_user
    ...
    echo 11 > pgversions
    echo 9 > compat
    echo "percona-pgaudit11-set-user (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
    echo "  * Initial Release." >> changelog
    echo " -- EvgeniyPatlan <evgeniy.patlan@percona.com> $(date -R)" >> changelog
    
    wget https://raw.githubusercontent.com/sudokamikaze/postgres-packaging/11.12/pgaudit_set_user/control
    wget https://raw.githubusercontent.com/sudokamikaze/postgres-packaging/11.12/pgaudit_set_user/control.in
    wget https://raw.githubusercontent.com/sudokamikaze/postgres-packaging/11.12/pgaudit_set_user/copyright
    wget https://raw.githubusercontent.com/sudokamikaze/postgres-packaging/11.12/pgaudit_set_user/rules
    cd ../ 
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/sudokamikaze/postgres-packaging/11.12/pgaudit_set_user/percona-pgaudit11_set_user.spec
    cd ${WORKDIR}
    #
...
build_srpm(){
    ...
    cp -av rpm/percona-pgaudit11_set_user.spec rpmbuild/SPECS
    #
    mv -fv ${TARFILE} ${WORKDIR}/rpmbuild/SOURCES
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" rpmbuild/SPECS/percona-pgaudit11_set_user.spec
...
build_rpm(){
    ...
    rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "pgmajorversion 11" --define "dist .$OS_NAME" --define "version ${VERSION}" --rebuild rpmbuild/SRPMS/$SRC_RPM
...
build_source_deb(){
    ...
    #
    PRODUCT_DEB="percona-pgaudit11-set-user"
...
build_deb(){
    ...
    dpkg-source -x ${DSC}
    #
    cd percona-pgaudit11-set-user-${VERSION}
