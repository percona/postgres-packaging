...
get_sources(){
    ...
    rm -f control
    wget https://raw.githubusercontent.com/percona/postgres-packaging/15.1/pgbackrest/control
    wget https://raw.githubusercontent.com/percona/postgres-packaging/15.1/pgbackrest/compat
    wget https://raw.githubusercontent.com/percona/postgres-packaging/15.1/pgbackrest/rules.patch
    ...
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/percona/postgres-packaging/15.1/pgbackrest/pgbackrest.spec
    wget https://raw.githubusercontent.com/percona/postgres-packaging/15.1/pgbackrest/pgbackrest.conf
    cd ${WORKDIR}
    #
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      INSTALL_LIST="percona-postgresql-common percona-postgresql15-devel git rpm-build rpmdevtools systemd systemd-devel wget libxml2-devel openssl-devel perl perl-libxml-perl perl-DBD-Pg perl-Digest-SHA perl-IO-Socket-SSL perl-JSON-PP zlib-devel gcc make autoconf perl-ExtUtils-Embed"
