...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      INSTALL_LIST="percona-postgresql13-devel git rpm-build rpmdevtools systemd systemd-devel wget bzip2-devel libxml2-devel openssl-devel perl perl-DBD-Pg perl-Digest-SHA perl-IO-Socket-SSL perl-JSON-PP zlib-devel gcc make autoconf perl-ExtUtils-Embed libssh2-devel libzstd-devel lz4-devel"
