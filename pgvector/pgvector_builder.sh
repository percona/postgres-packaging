...
get_sources(){
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${PG_VERSION}/${REVISION}/${BUILD_ID}" >> pgvector.properties
...
get_tar(){
    TARBALL=$1
    TARFILE=$(basename $(find $WORKDIR/$TARBALL -name 'percona-pgvector*.tar.gz' | sort | tail -n1))
    if [ -z $TARFILE ]
    then
        TARFILE=$(basename $(find $CURDIR/$TARBALL -name 'percona-pgvector*.tar.gz' | sort | tail -n1))
        if [ -z $TARFILE ]
        then
