    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      if [ x"$RHEL" = x6 -o x"$RHEL" = x7 ]; then
         ...
         if [ x"$RHEL" = x8 ]; then
             ...
             dnf update
             # Commenting following line because llvm must install from percona repo with percona-postgresql15-devel
             #yum -y install llvm-toolset llvm-devel
	 else
             yum -y install llvm-toolset llvm-devel clang
	 fi
         INSTALL_LIST="git rpm-build  autoconf libtool flex rpmdevtools wget rpmlint percona-postgresql15-devel gcc make  geos geos-devel proj libgeotiff-devel pcre-devel gmp-devel geos311-devel gmp-devel gtk2-devel json-c-devel libgeotiff17-devel proj90-devel protobuf-c-devel pkg-config"
         yum -y install ${INSTALL_LIST}
