...
get_sources(){
    ...
    cd debian
        ...
        wget https://raw.githubusercontent.com/percona/postgres-packaging/${PPG_VERSION}/postgis/debian/percona-postgresql-14-postgis-3-scripts.lintian-overrides
        wget https://raw.githubusercontent.com/percona/postgres-packaging/${PPG_VERSION}/postgis/debian/percona-postgresql-14-postgis-3-scripts.postinst
        wget https://raw.githubusercontent.com/percona/postgres-packaging/${PPG_VERSION}/postgis/debian/percona-postgresql-14-postgis-3-scripts.prerm
        wget https://raw.githubusercontent.com/percona/postgres-packaging/${PPG_VERSION}/postgis/debian/percona-postgresql-14-postgis-3.install
...
install_sfcgal(){
    ...
    cp /usr/local/lib/libSFCGAL.* /usr/lib/postgresql/14/lib
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      if [ x"$RHEL" = x6 -o x"$RHEL" = x7 ]; then
         ...
         if [ x"$RHEL" = x8 ]; then
             ...
             dnf update
             # Commenting following line because llvm must install from percona repo with percona-postgresql14-devel
             #yum -y install llvm-toolset llvm-devel
	 else
             yum -y install llvm-toolset llvm-devel clang
	 fi
         INSTALL_LIST="git rpm-build  autoconf libtool flex rpmdevtools wget rpmlint percona-postgresql14-devel gcc make  geos geos-devel proj libgeotiff-devel pcre-devel gmp-devel geos311-devel gmp-devel gtk2-devel json-c-devel libgeotiff17-devel proj90-devel protobuf-c-devel pkg-config"
         yum -y install ${INSTALL_LIST}
...
build_srpm(){
    ...
    cd rpmbuild/SOURCES
    wget https://raw.githubusercontent.com/percona/postgres-packaging/${PPG_VERSION}/postgis/postgis-3.3.7.pdf
    #wget --no-check-certificate https://download.osgeo.org/postgis/docs/postgis-3.3.7.pdf
    ...
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" --define "pginstdir /usr/pgsql-14"  \
...
build_rpm(){
    ...
    rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .$OS_NAME" --define "version ${VERSION}" --define "pginstdir /usr/pgsql-14" --rebuild rpmbuild/SRPMS/$SRC_RPM
