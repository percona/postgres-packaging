...
get_sources(){
    ...
    cd rpm
        ...
        wget https://raw.githubusercontent.com/percona/postgres-packaging/${PPG_VERSION}9/postgis/rpm/percona-postgis33.spec
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      if [ x"$RHEL" = x6 -o x"$RHEL" = x7 ]; then
         ...
         INSTALL_LIST="xerces-c-devel clang-devel clang llvm-devel git rpm-build  autoconf libtool flex rpmdevtools wget rpmlint percona-postgresql14-devel gcc make  geos geos-devel libgeotiff-devel gmp-devel gmp-devel gtk2-devel json-c-devel libgeotiff17-devel protobuf-c-devel pkg-config"
         yum -y install ${INSTALL_LIST}
