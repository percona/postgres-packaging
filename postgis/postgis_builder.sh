...
install_sfcgal(){
    ...
    cp /usr/local/lib/libSFCGAL.* /usr/lib/postgresql/11/lib
...
#      mv -f percona-dev.repo /etc/yum.repos.d/
	 ...
	 dnf module -y disable postgresql
      ...
      percona-release enable-only ppg-11.19 testing
      apt-get update
...
build_srpm(){
    ...
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" --define "pginstdir /usr/pgsql-11"  \
...
build_rpm(){
    ...
    rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .$OS_NAME" --define "version ${VERSION}" --define "pginstdir /usr/pgsql-11" --rebuild rpmbuild/SRPMS/$SRC_RPM
