...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      if [ x"$RHEL" = x6 -o x"$RHEL" = x7 ]; then
         ...
         INSTALL_LIST="clang-devel clang llvm-devel git rpm-build  autoconf libtool flex rpmdevtools wget rpmlint percona-postgresql14-devel gcc make  geos geos-devel libgeotiff-devel pcre-devel gmp-devel geos311-devel gmp-devel gtk2-devel json-c-devel libgeotiff17-devel protobuf-c-devel pkg-config"
         yum -y install ${INSTALL_LIST}
