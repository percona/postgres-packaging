...
get_sources(){
	...
	rm -rf rules control supported-versions 
        wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/control
        wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/maintscripts-functions.patch
        wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/percona-postgresql-common.templates.patch
        wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/rules
	wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/supported-versions
    ...
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/pgcommon.sh
    cd rpm
        for file in $(ls | grep postgresql); do
            mv $file "percona-$file"
        done
	rm -rf percona-postgresql-common.spec
        wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/postgres-common/percona-postgresql-common.spec
    cd ../
    cd ${WORKDIR}
    #
...
build_rpm(){
    ...
    RHEL=$(rpm --eval %rhel)
    ARCH=$(echo $(uname -m) | sed -e 's:i686:i386:g')
    rpmbuild -D 'debug_package %{nil}' --define "_topdir ${WORKDIR}/rpmbuild" --define "version ${VERSION}" --define "dist .$OS_NAME" --rebuild rpmbuild/SRPMS/$SRC_RPM
