...
get_sources(){
    ...
    mkdir rpm
    mv pgrpms/rpm/redhat/main/non-common/postgresql-14/main/*   rpm/
    rm -rf pgrpms
    cd rpm
        rm postgresql-14.spec
        wget  https://raw.githubusercontent.com/percona/postgres-packaging/${PG_VERSION}/postgres/percona-postgresql-14.spec
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}-14/${PRODUCT_FULL}/${PSM_BRANCH}/${REVISION}/${DATE_TIMESTAMP}/${BUILD_ID}" >> percona-postgresql.properties
...
build_srpm(){
    ...
    line_number=$(grep -n SOURCE999 percona-postgresql-14.spec | awk -F ':' '{print $1}')
    cp ../SOURCES/call-home.sh ./
    awk -v n=$line_number 'NR <= n {print > "part1.txt"} NR > n {print > "part2.txt"}' percona-postgresql-14.spec
    ...
    mv part1.txt percona-postgresql-14.spec
    cd ${WORKDIR}
...
build_deb(){
    ...
    unset $(locale|cut -d= -f1)
        cd debian/
        wget https://raw.githubusercontent.com/Percona-Lab/telemetry-agent/phase-0/call-home.sh
        sed -i 's:exit 0::' percona-postgresql-14.postinst
        echo "cat <<'CALLHOME' > /tmp/call-home.sh" >> percona-postgresql-14.postinst
        cat call-home.sh >> percona-postgresql-14.postinst
        echo "CALLHOME" >> percona-postgresql-14.postinst
        echo "bash +x /tmp/call-home.sh -f \"PRODUCT_FAMILY_POSTGRESQL\" -v \"${PG_VERSION}-${DEB_RELEASE}\" -d \"PACKAGE\" || :" >> percona-postgresql-14.postinst
	echo "chgrp percona-telemetry /usr/local/percona/telemetry_uuid &>/dev/null || :" >> percona-postgresql-14.postinst
        echo "chmod 664 /usr/local/percona/telemetry_uuid &>/dev/null || :" >> percona-postgresql-14.postinst
        echo "rm -rf /tmp/call-home.sh" >> percona-postgresql-14.postinst
        echo "exit 0" >> percona-postgresql-14.postinst
        rm -f call-home.sh
    cd ../
