...
get_sources(){
    ...
    cd deb_packaging
        git checkout -b 11 debian/11.22-1
    cd ../
	...
	rm -f control rules
        wget https://raw.githubusercontent.com/percona/postgres-packaging/11.22/postgres/control
        wget https://raw.githubusercontent.com/percona/postgres-packaging/11.22/postgres/rules
    ...
    mkdir rpm
    mv pgrpms/rpm/redhat/main/non-common/postgresql-11/main/* rpm/
    rm -rf pgrpms
    cd rpm
        rm postgresql-11.spec
        wget https://raw.githubusercontent.com/percona/postgres-packaging/11.22/postgres/percona-postgresql-11.spec
    cd ../
    cd ${WORKDIR}
    #
...
build_srpm(){
    ...
    cd rpmbuild/SOURCES
    wget --no-check-certificate https://www.postgresql.org/files/documentation/pdf/11/postgresql-11-A4.pdf
    cd ../../
    ...
    line_number=$(grep -n SOURCE999 percona-postgresql-11.spec | awk -F ':' '{print $1}')
    cp ../SOURCES/call-home.sh ./
    awk -v n=$line_number 'NR <= n {print > "part1.txt"} NR > n {print > "part2.txt"}' percona-postgresql-11.spec
    ...
    mv part1.txt percona-postgresql-11.spec
    cd ${WORKDIR}
...
build_rpm(){
    ...
    rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .$OS_NAME" --define "pgmajorversion 11" --define "pginstdir /usr/pgsql-11" --define "pgpackageversion 11" --rebuild rpmbuild/SRPMS/$SRC_RPM
...
build_deb(){
    ...
    unset $(locale|cut -d= -f1)
        cd debian/
        wget https://raw.githubusercontent.com/Percona-Lab/telemetry-agent/phase-0/call-home.sh
        sed -i 's:exit 0::' percona-postgresql-11.postinst
        echo "cat <<'CALLHOME' > /tmp/call-home.sh" >> percona-postgresql-11.postinst
        cat call-home.sh >> percona-postgresql-11.postinst
        echo "CALLHOME" >> percona-postgresql-11.postinst
        echo 'bash +x /tmp/call-home.sh -f "PRODUCT_FAMILY_POSTGRESQL" -v "11.22" -d "PACKAGE" || :' >> percona-postgresql-11.postinst
        echo "rm -rf /tmp/call-home.sh" >> percona-postgresql-11.postinst
        echo "exit 0" >> percona-postgresql-11.postinst
        rm -f call-home.sh
    cd ../
