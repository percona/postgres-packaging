...
if [ $# -ne 4 ];
then
    echo "Usage: $0 <Install dir> <Version> <Branding> <Temp dir>"
    exit 127
fi
...
"$INSTALLDIR/installer/xdg/xdg-desktop-menu" uninstall --mode system --noupdate \
          "$INSTALLDIR/scripts/xdg/pg-doc-postgresql-$VERSION_STR.desktop"  || _warn "Failed to remove the documentation menu"

"$INSTALLDIR/installer/xdg/xdg-desktop-menu" uninstall --mode system \
	  "$INSTALLDIR/scripts/xdg/pg-psql-$VERSION_STR.desktop" || _warn "Failed to remove the top level menu"
