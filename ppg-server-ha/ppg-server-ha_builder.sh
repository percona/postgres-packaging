...
get_sources(){
    ...
    PRODUCT=percona-ppg-server-ha-12
    ...
    mkdir debian
    cd debian/
    wget https://raw.githubusercontent.com/percona/postgres-packaging/12.13/ppg-server-ha/control
    wget https://raw.githubusercontent.com/percona/postgres-packaging/12.13/ppg-server-ha/rules
    echo 9 > compat
    echo "percona-ppg-server-ha-12 (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
    ...
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/percona/postgres-packaging/12.13/ppg-server-ha/ppg-server-ha.spec
    cd ${WORKDIR}
    #
