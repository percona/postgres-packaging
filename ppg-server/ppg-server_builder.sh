...
get_sources(){
    ...
    mkdir debian
    cd debian/
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/ppg-server/control
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/ppg-server/rules
    echo 9 > compat
    echo "percona-ppg-server-11 (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
    ...
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.21/ppg-server/ppg-server.spec
    cd ${WORKDIR}
    #
