...
get_sources(){
    ...
    mkdir debian
    cd debian/
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/ppg-server/control
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/ppg-server/rules
    echo 9 > compat
    echo "percona-ppg-server-13 (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
    ...
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/percona/postgres-packaging/13.11/ppg-server/ppg-server.spec
    cd ${WORKDIR}
    #
