...
get_sources(){
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${PSM_BRANCH}/${REVISION}/${BUILD_ID}" >> timescaledb.properties
...
build_rpm(){
    ...
    SRC_RPM=$(basename $(find $WORKDIR/srpm -name 'percona-timescaledb*.src.rpm' | sort | tail -n1))
    if [ -z $SRC_RPM ]
    then
        SRC_RPM=$(basename $(find $CURDIR/srpm -name 'percona-timescaledb*.src.rpm' | sort | tail -n1))
        if [ -z $SRC_RPM ]
        then
...
CURDIR=$(pwd)
VERSION_FILE=$CURDIR/timescaledb.properties
args=
WORKDIR=
SRPM=0
SDEB=0
