...
get_sources(){
    ...
    rm -f control rules
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.14/wal2json/control
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.14/wal2json/control.in
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.14/wal2json/rules
    echo 11 > pgversions
    echo 9 > compat
    cd ../
    rm -rf deb_packaging
    mkdir rpm
    cd rpm
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.14/wal2json/percona-wal2json.spec
    wget https://raw.githubusercontent.com/EvgeniyPatlan/postgres-packaging/11.14/wal2json/wal2json-pg11-makefile-pgxs.patch
    cd ${WORKDIR}
    #
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      add_percona_apt_repo
      apt-get update || true
      INSTALL_LIST="build-essential clang-11 pkg-config liblz4-dev debconf debhelper devscripts dh-exec git wget libxml-checker-perl libxml-libxml-perl libio-socket-ssl-perl libperl-dev libssl-dev libxml2-dev txt2man zlib1g-dev libpq-dev percona-postgresql-11 percona-postgresql-common percona-postgresql-server-dev-all percona-postgresql-all libbz2-dev libzstd-dev libevent-dev libssl-dev libc-ares-dev pandoc pkg-config"
